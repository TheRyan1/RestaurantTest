...
namespace reactnet.Controllers
{
    ...
    public class ReservationController : ControllerBase
    {
        ...
        public ReservationController(ApplicationDbContext dbContext) => _dbContenxt = dbContext;
        ...
        public async Task<List<Reservation>> Get(int id, int reservationID)
        {
            ...
            if (user.Role == UserRoleConstants.ADMIN)
            {
                ...
                return id != 0 ?
                await _dbContenxt.Reservation.Where(x => x.RestaurantID == id).Include(x => x.Restaurant).Include(x => x.Orders).ToListAsync() :
            ...
            return id != 0 ?
            await _dbContenxt.Reservation.Where(x => x.RestaurantID == id).Include(x => x.Restaurant).Include(x => x.Orders).ToListAsync() :
            await _dbContenxt.Reservation.Where(x => x.Restaurant.UserID == user.Id).Include(x => x.Restaurant).Include(x => x.Orders).ToListAsync();
        }
