...
namespace reactnet.Controllers
{
    ...
    public class MealsController : ControllerBase
    {
        ...
        public MealsController(ApplicationDbContext dbContext) => _dbContenxt = dbContext;
        ...
        public async Task<List<Meal>> Get(int id)
        {
            ...
            return id != 0 ?
            await _dbContenxt.Meal.Where(x => x.RestaurantID == id && x.Restaurant.UserID == user.Id).Include(x => x.Restaurant).ToListAsync() :
            await _dbContenxt.Meal.Where(x => x.Restaurant.UserID == user.Id).Include(x => x.Restaurant).ToListAsync();
        }
